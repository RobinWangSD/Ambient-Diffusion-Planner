...
class DiffusionPlannerDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        if stage == "fit" or stage is None:
            # Training dataset (existing code)
...
def get_args():
    ...
    parser.add_argument('--train_mapping_pkl', type=str, required=True,
                        help='Path to the pickle file containing file mappings')
    parser.add_argument('--train_prefix', type=str, default=None,
                        help='Optional prefix to filter training files (e.g., "train_")')
    parser.add_argument('--val_set', type=str, help='path to validation data', default=None)
    parser.add_argument('--val_mapping_pkl', type=str, required=True,
                        help='Path to the pickle file containing file mappings')
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='{epoch}-{step}-{val_loss:.4f}' if args.val_set else '{step}-{train_loss:.4f}',
        monitor='val_loss' if args.val_set else 'train_loss',
        mode='min',
        save_top_k=-1, 
