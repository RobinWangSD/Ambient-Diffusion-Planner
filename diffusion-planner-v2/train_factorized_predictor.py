...
from diffusion_planner.utils.tb_log import UnifiedLogger as Logger
...
from diffusion_planner.train_epoch_factorized import train_epoch_factorized
...
def get_args():
    ...
    parser.add_argument('--mapping_pkl', type=str, required=True,
                        help='Path to the pickle file containing file mappings')
    ...
    parser.add_argument('--action_type', type=str, help='number of time point', default='traj') # ['traj', 'vel', 'ik']
    ...
    parser.add_argument('--logger', default='qualcomm', type=str, choices=['none', 'qualcomm', ' wandb', 'tensorboard'])
...
def model_training(args):
    ...
    train_set = DiffusionPlannerDataDistributed(
        data_dir=args.train_set,  # Directory where batch_* folders are located
        mapping_pkl=args.mapping_pkl,  # Path to the pickle mapping file
        past_neighbor_num=args.agent_num,  # Using agent_num as past_neighbor_num
        predicted_neighbor_num=args.predicted_neighbor_num,
        future_len=args.future_len,
        filter_prefix=args.train_prefix if hasattr(args, 'train_prefix') else None,  # Optional filtering
        max_files=int(args.max_files),
    )

    if args.use_batch_aware:
        train_sampler = BatchAwareDistributedSampler(
            train_set, 
        ...
        # Fallback to standard distributed sampler
    ...
    # set up model
    diffusion_planner = Factorized_Diffusion_Planner(args)
    ...
    # logger
    wandb_logger = Logger(args, args.name, save_path=save_path, rank=global_rank, wandb_id=wandb_id) 
    ...
    for epoch in range(init_epoch, train_epochs):
        ...
        # Pass logger and epoch to train_epoch for step-wise logging
        train_loss, train_total_loss = train_epoch_factorized(
            train_loader, 
            diffusion_planner, 
            optimizer, 
            args, 
            model_ema, 
            aug,
            logger=wandb_logger if global_rank == 0 else None,  # Only pass logger to rank 0
            epoch=epoch
        )

        if global_rank == 0:
            lr_dict = {'lr': optimizer.param_groups[0]['lr']}
            # Keep epoch-wise logging as well
