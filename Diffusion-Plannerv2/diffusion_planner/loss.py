...
def diffusion_loss_factorized_func(
    model: nn.Module,
    inputs: Dict[str, torch.Tensor],
    marginal_prob: Callable[[torch.Tensor], torch.Tensor],
    ...
    ego_future, neighbors_future, neighbor_future_mask = futures
    neighbors_future_valid = ~neighbor_future_mask # [B, P-1, V]
    ...
    all_gt = norm(gt_future)
    all_gt[:, 1:][neighbor_future_mask] = 0.0

    mean, std = marginal_prob(all_gt, t)
    ...
    if model_type == "score":
        ...
        diff = score - all_gt
        diff[:, 1:, :, :][neighbor_future_mask] = 0.
        dpm_loss = torch.sum(diff**2, dim=-1)
