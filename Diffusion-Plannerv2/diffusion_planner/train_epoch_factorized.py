...
def train_epoch_factorized(data_loader, model, optimizer, args, ema, aug: StatePerturbation=None, logger=None, epoch=None):
    epoch_loss = []
    step_count = 0  # Track step within epoch
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            # Log step-wise losses (only on rank 0 to avoid duplicate logs)
            if logger is not None and ddp.get_rank() == 0 and epoch is not None:
                global_step = epoch * len(data_loader) + step_count
                step_loss_dict = {k: v.item() if torch.is_tensor(v) else v for k, v in loss.items()}
                logger.log_metrics({f"step_loss/{k}": v for k, v in step_loss_dict.items()}, step=global_step)
