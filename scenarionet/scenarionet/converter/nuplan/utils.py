...
def get_nuplan_scenarios(data_root, map_root, logs: Union[list, None] = None, builder="nuplan_mini", num_db_workers=80):
    """
    ...
    overrides = [
        ...
        f'worker.max_workers={num_db_workers}',
...
def convert_nuplan_scenario(scenario: NuPlanScenario, version):
    ...
    # map
    ego_track = result[SD.TRACKS][EGO]['state']['position']
    x_dis = ego_track[:,0].max() - ego_track[:,0].min()
    y_dis = ego_track[:,1].max() - ego_track[:,1].min()
    traverse_dist = np.linalg.norm([x_dis, y_dis])
    result[SD.MAP_FEATURES] = extract_map_features(scenario.map_api, scenario_center, route_block_ids, radius=traverse_dist+200)
