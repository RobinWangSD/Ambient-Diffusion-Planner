...
if __name__ == '__main__':
    ...
    parser.add_argument("--num_db_workers", type=int, default=8, help="number of workers for nuplan db access")
    parser.add_argument(
        "--raw_data_path",
        type=str,
    ...
    num_db_workers = args.num_db_workers

    data_root = args.raw_data_path
    map_root = os.getenv("NUPLAN_MAPS_ROOT")
    if args.test:
        scenarios = get_nuplan_scenarios(data_root, map_root, logs=["2021.07.16.20.45.29_veh-35_01095_01486"], num_db_workers=num_db_workers)
    else:
        scenarios = get_nuplan_scenarios(data_root, map_root, num_db_workers=num_db_workers)
