...
def process_scenario(
    scenario, num_past_poses, past_time_horizon, num_agents, num_static,
...
class DataProcessor(object):
    ...
    def parallel_work(self, scenarios: List[AbstractScenario]):
        # Create a pool of workers
        num_processes = multiprocessing.cpu_count()  # or set a fixed number if preferred

        # Extract attributes to avoid pickling issues
        num_past_poses = self.num_past_poses
        past_time_horizon = self.past_time_horizon
        ...
        args = [(scenario, num_past_poses, past_time_horizon, num_agents,
                num_static, max_ped_bike, map_features, radius,
                max_elements, max_points, num_future_poses,
                future_time_horizon, save_dir) for scenario in scenarios]
