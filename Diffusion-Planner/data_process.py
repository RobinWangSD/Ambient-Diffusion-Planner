...
if __name__ == "__main__":
    ...
    parser.add_argument('--id', type=int, help='number of agents', default=0)
    ...
    with open(f'/root/planning-intern/Diffusion-Planner/train_splits/nuplan_train_{args.id:02d}.json', "r", encoding="utf-8") as file:
    ...
    scenario_filter = ScenarioFilter(*get_filter_parameters(args.scenarios_per_type, total_scenarios, args.shuffle_scenarios, log_names=log_names))
    ...
    processor.parallel_work(scenarios)

    npz_files = [f for f in os.listdir(args.save_path) if f.endswith('.npz')]

    # Save the list to a JSON file
    with open(f'/data/temp/Expire180Days/users/luobwang/nuplan_processed/diffusion_planner/diffusion_planner_training_{args.id}.json', 'w') as json_file:
