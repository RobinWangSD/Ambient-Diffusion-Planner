...
class EgoCentricFlowEncoder(nn.Module):
    '''
    Flow Model Ego Centric Encoder using Latent Query Attention.
    '''

    def __init__(self, config):
        super(EgoCentricFlowEncoder, self).__init__()
        self.config = config
...
class EgoCentricFlowDecoder(nn.Module):
    """
    Flow model decoder that takes in ego centric scene embdedings from the encoder.
    """
    def __init__(self, config):
        super(EgoCentricFlowDecoder, self).__init__()
        self.config = config
    ...
    def forward(self, x, t, encodings, curr_states, agent_mask, y_goal=None, y_route=None):
        ...
        x_embedding = x_embedding[None, :, None, :].expand(B, -1, self.chunk_steps, -1) # (B, A, T_chunk, h)
        x = x + x_embedding     # (B, A, T_chunk, h)

        curr_states_embedding = self.agent_pos_embedding(curr_states)   # (B, A, h)
        curr_states_embedding = curr_states_embedding[:, :, None, :].expand(-1, -1, self.chunk_steps, -1)   # (B, A, T_chunk, h)
        x = x + curr_states_embedding
