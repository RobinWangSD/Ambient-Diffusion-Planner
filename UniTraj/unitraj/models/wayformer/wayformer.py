...
class Wayformer(BaseModel):
    ...
    def _forward(self, inputs):
        ...
        out_seq = self.perceiver_decoder(context)         # (B, N_query_dec, h)
    ...
    def forward(self, batch):
        ...
        ego_mask = torch.gather(agents_mask, 1, inputs['track_index_to_predict'].view(-1, 1, 1).repeat(1, 1,
                                                                                                       agents_mask.shape[
                                                                                                           -1])).squeeze(
            1)
        agents_in = torch.cat([agents_in, agents_mask.unsqueeze(-1)], dim=-1)   # (B, N_cloest_agents, T_h, 30)
        agents_in = agents_in.transpose(1, 2)                                   # (B, T_h, N_ca, 30)
    ...
    def configure_optimizers(self):
        ...
        # scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.0002, steps_per_epoch=1, epochs=self.config['max_epochs'],
