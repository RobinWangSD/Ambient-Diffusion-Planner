...
class PerceiverDecoder(nn.Module):
    ...
    def forward(self, x_latent, x_adapted=None, **kwargs):
        output_query = self.output_query_provider(x_adapted)                        # (1, N_query_dec, h)

        output = self.cross_attn[0](output_query, x_latent).last_hidden_state       # (B, N_query_dec, h)

        for i in range(1, len(self.cross_attn)):
            output = self.self_attn[i - 1](output).last_hidden_state                # (B, N_query_dec, h)
            output = self.cross_attn[i](output, x_latent).last_hidden_state         # (B, N_query_dec, h)
...
class ChunkEmbedder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.proj = nn.Conv2d(feature_size, hidden_dim, kernel_size=(1,chunk_size), stride=(1,chunk_size), bias=bias)
...
class FactorizedDiTBlock(nn.Module):
    """
    A DiT block with adaptive layer norm zero (adaLN-Zero) conditioning for ego and Factorized Attention.
    """
    def __init__(self, dim=192, heads=6, dropout=0.1, mlp_ratio=4.0):
        super(FactorizedDiTBlock, self).__init__()
        self.heads = heads

        # temporal attention with AdaptiveLN Zero
        self.norm1 = nn.LayerNorm(dim)
        self.temporal_attn = nn.MultiheadAttention(dim, heads, dropout, batch_first=True)
        self.norm2 = nn.LayerNorm(dim)
        self.mlp1 = MLP(dim, mlp_ratio, bias=True)
        ...
        self.mlp2 = MLP(dim, mlp_ratio, bias=True)
        ...
        self.mlp3 = MLP(dim, mlp_ratio, bias=True)

    def forward(self, x, encodings, y, agent_mask):
        B, A, T, h = x.shape

        # causal
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation1(y).chunk(6, dim=1)  #(B, h)

        causal_mask = torch.triu(torch.ones(T, T), diagonal=1).bool().to(x.device)
        modulated_x = modulate(self.norm1(x), shift_msa, scale_msa).view(B*A, T, h)
        x_attn = self.temporal_attn(modulated_x, modulated_x, modulated_x, attn_mask=causal_mask)[0]
        x = x + gate_msa.unsqueeze(1).unsqueeze(1) * x_attn.view(B, A, T, h)

        modulated_x = modulate(self.norm2(x), shift_mlp, scale_mlp)
        x = x + gate_mlp.unsqueeze(1).unsqueeze(1) * self.mlp1(modulated_x).last_hidden_state

        # agent
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation2(y).chunk(6, dim=1)
        x = x.permute(0, 2, 1, 3)       # (B, T, A, h)

        modulated_x = modulate(self.norm3(x), shift_msa, scale_msa).view(B*T, A, h)
        agent_mask = agent_mask.unsqueeze(1).expand(-1, T, -1).to(x.device).contiguous()
        agent_mask = agent_mask.view(B*T, A)

        agent_attn_mask = agent_mask.unsqueeze(1) * agent_mask.unsqueeze(2)
        agent_attn_mask = agent_attn_mask.repeat_interleave(self.heads, dim=0)
        x_attn = self.agent_attn(modulated_x, modulated_x, modulated_x, attn_mask=agent_attn_mask)[0]
        x = x + gate_msa.unsqueeze(1).unsqueeze(1) * x_attn.view(B, T, A, h) 

        modulated_x = modulate(self.norm4(x), shift_mlp, scale_mlp)
        x = x + gate_mlp.unsqueeze(1).unsqueeze(1) * self.mlp2(modulated_x).last_hidden_state
        x = x.permute(0, 2, 1, 3)       # (B, A, T, h)

        # historical
        x = x + self.history_attn(self.norm5(x.view(B, A*T, h)), encodings, encodings)[0].view(B, A, T, h)
        x = x + self.mlp3(self.norm6(x)).last_hidden_state
...
from timm.models.layers import Mlp as timm_mlp
class MixerBlock(nn.Module):
    def __init__(self, tokens_mlp_dim, channels_mlp_dim, drop_path_rate):
        super().__init__()

        self.norm1 = nn.LayerNorm(channels_mlp_dim)
        self.channels_mlp = timm_mlp(in_features=channels_mlp_dim, hidden_features=channels_mlp_dim, act_layer=nn.GELU, drop=drop_path_rate)
        self.norm2 = nn.LayerNorm(channels_mlp_dim)
        self.tokens_mlp = timm_mlp(in_features=tokens_mlp_dim, hidden_features=tokens_mlp_dim, act_layer=nn.GELU, drop=drop_path_rate)
...
class RouteEncoder(nn.Module):
    def __init__(self, route_num, lane_len, drop_path_rate=0.3, hidden_dim=192, tokens_mlp_dim=32, channels_mlp_dim=64):
        ...
        self.channel_pre_project = timm_mlp(in_features=4, hidden_features=channels_mlp_dim, out_features=channels_mlp_dim, act_layer=nn.GELU, drop=0.)
        self.token_pre_project = timm_mlp(in_features=route_num * lane_len, hidden_features=tokens_mlp_dim, out_features=tokens_mlp_dim, act_layer=nn.GELU, drop=0.)

        self.Mixer = MixerBlock(tokens_mlp_dim, channels_mlp_dim, drop_path_rate)

        self.norm = nn.LayerNorm(channels_mlp_dim)
        self.emb_project = timm_mlp(in_features=channels_mlp_dim, hidden_features=hidden_dim, out_features=hidden_dim, act_layer=nn.GELU, drop=drop_path_rate)
