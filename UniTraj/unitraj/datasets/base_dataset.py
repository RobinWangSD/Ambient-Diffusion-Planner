...
class BaseDataset(Dataset):
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            self.cache_path = os.path.join(
                ...
                '' if observe_type is None and predict_type is None else f'observe_{observe_type}' + f'predict_{predict_type}',
                f"{self.config['max_num_agents']}_agents", 
                dataset_name, phase
                )
    ...
    def preprocess(self, scenario):
        ...
        for k, v in tracks.items():
            ...
            all_state = np.concatenate(all_state, axis=-1)   # (T, S=10)
        ...
        if self.config['only_train_on_ego']:  # TODO: change to match the prediction type (ego vehicle and surrounding vehicles)
            tracks_to_predict = {
    ...
    def process(self, internal_format):
        ...
        if center_objects is None: return None      # (N-obj, S)
    ...
    def get_agent_data(
        ...
        center_gt_trajs = obj_trajs_future_state[center_obj_idxs, track_index_to_predict]   # (N_sa, T_f, 4)
        center_gt_trajs_mask = obj_trajs_future_mask[center_obj_idxs, track_index_to_predict] # (N_sa, T_f)
    ...
    def get_interested_agents(self, track_index_to_predict, obj_trajs_full, current_time_index, obj_types, scene_id):
        ...
        for k in range(len(track_index_to_predict)):
            ...
            center_objects_list.append(obj_trajs_full[obj_idx, current_time_index])  # current state of the object
    ...
    def trajectory_filter(self, data):
        ...
        for idx,(k,v) in enumerate(obj_summary.items()):
            ...
            if v_type not in ['VEHICLE', 'PEDESTRIAN', 'CYCLIST']: continue
            # valid_ratio = v['valid_length']/v['track_length']
            # if valid_ratio < 0.5: continue
            # moving_distance = v['moving_distance']
            # if moving_distance < 2.0 and v_type=='VEHICLE': continue
            ...
            tracks_to_preidct[k] = {'track_index': idx, 'track_id': k, 'difficulty': 0, 'object_type': v_type}
