...
def get_kalman_difficulty(output):
    ...
    for data_sample in output:
        ...
        kalman_difficulty_2s, kalman_difficulty_4s, kalman_difficulty_6s, kalman_difficulty_8s = -1, -1, -1, -1
        try:
            if valid_future >= 19:
                ...
                if valid_future >= 39:
                    ...
                    if valid_future >= 59:
                        ...
                        if valid_future >= 79:
                            kalman_8s = estimate_kalman_filter(past_trajectory_valid, 80)  # (x,y)
                            gt_future_8s = gt_future[79, :]
                            kalman_difficulty_8s = calculate_epe(kalman_8s, gt_future_8s)
        except:
            kalman_difficulty_2s, kalman_difficulty_4s, kalman_difficulty_6s, kalman_difficulty_8s = -1, -1, -1, -1
        data_sample["kalman_difficulty"] = np.array(
            [
                kalman_difficulty_2s, 
                kalman_difficulty_4s, 
                kalman_difficulty_6s,
                kalman_difficulty_8s,
            ]
            )
    return

def get_kalman_difficulty_multi_agent(output):
    """
    return the kalman difficulty at 2s, 4s, and 6s
    if the gt future is not valid up to the considered second, the difficulty is set to -1
    """
    for data_sample in output:
        kalman_difficulty_multi_agent = []
        for i in range(data_sample['obj_trajs_future_state'].shape[0]):
            past_trajectory = data_sample["obj_trajs"][i, :, :2].copy()  # Time X (x,y)
            past_mask = data_sample["obj_trajs_mask"][i, :].copy()

            valid_past = count_valid_steps_past(past_mask)
            past_trajectory_valid = past_trajectory[-valid_past:, :]  # Time(valid) X (x,y)

            # future gt trajectory of agent of interest
            gt_future = data_sample["obj_trajs_future_state"][i, :, :2].copy()  # Time x (x, y)
            # Get last valid position
            valid_future = int(data_sample["obj_trajs_future_final_valid_idx"][i])
            ...
            if valid_future >= 0:
                try:
                    ...
                    kalman_difficulty_2s, kalman_difficulty_4s, kalman_difficulty_6s, kalman_difficulty_8s = -1, -1, -1, -1
            kalman_difficulty_multi_agent.append(
                np.array(
                    [
                        kalman_difficulty_2s, 
                        kalman_difficulty_4s, 
        ...
        data_sample["kalman_difficulty_multi_agent"] = np.stack(kalman_difficulty_multi_agent, axis=0)
    return
...
def get_trajectory_type_multi_agent(output):
    for data_sample in output:
        traj_type_multi_agent = []
        for i in range(data_sample['obj_trajs_future_state'].shape[0]):
            traj_valid_mask = data_sample['obj_trajs_mask'][i, -1]
            if not traj_valid_mask:
                traj_type_multi_agent.append(-1)
                continue
            valid_end_point = int(data_sample["obj_trajs_future_final_valid_idx"][i])
            end_point = data_sample["obj_trajs_future_state"][i, valid_end_point, :2]  # (x,y)
            end_velocity = data_sample["obj_trajs_future_state"][i, valid_end_point, 3:5]  # (vx, vy)
            end_heading = wrap_angle(data_sample["obj_trajs_future_state"][i, valid_end_point, 2])

            # Get start position, velocity and heading.
            start_point = data_sample["obj_trajs"][i, -1, :2]  # (x,y)
            start_velocity = data_sample["obj_trajs"][i, -1, -4:-2]  # (vx, vy)
            start_heading = np.arctan2(
                data_sample["obj_trajs"][i, -1, -6],  # sin
                data_sample["obj_trajs"][i, -1, -5],  # cos
            )
            # print(start_heading - wrap_angle(data_sample["obj_trajs_future_state"][ind_to_pred, 0, 2]))
            ...
            try:
                ...
                trajectory_type = -1
            traj_type_multi_agent.append(trajectory_type)
        data_sample["trajectory_type_multi_agent"] = np.asarray(traj_type_multi_agent, dtype=np.int32)

    return

def get_actions_multi_agent(output, action_len=1, dt=0.1):
    for data_sample in output:
        obj_trajs_future_state = data_sample["obj_trajs_future_state"][:, :, :5].copy() # (x, y, yaw, vx, vy)
        assert obj_trajs_future_state.shape[-2] % action_len == 0, "future length should be divisible by action_len"
        # obtain the state information at the current time step
        curr_xy = data_sample["obj_trajs"][:, [-1], :2].copy()
        curr_yaw = np.arctan2(
                        data_sample["obj_trajs"][:, [-1], [-6]],  # sin
                        data_sample["obj_trajs"][:, [-1], [-5]],  # cos
                    )
        curr_vxvy = data_sample["obj_trajs"][:, [-1], -4:-2].copy()
        obj_trajs_curr_state = np.concatenate(
            [
                curr_xy,
                curr_yaw[:, :, None],
                curr_vxvy,
        ...
        gt_trajs = np.concatenate([obj_trajs_curr_state, obj_trajs_future_state], axis=-2)

        num_actions = obj_trajs_future_state.shape[-2] // action_len
        
        # compute inverse kinematics actions and vx_vy_dyaw actions
        # (reference: https://github.com/SafeRoboticsLab/VBD/blob/61251efe4f1f7bbaac18e4469ef11145c04e2d9c/vbd/model/model_utils.py#L119)
        yaw = gt_trajs[:, :, 2].copy()     # (A, T)
        speed = np.linalg.norm(gt_trajs[:, :, 3:], axis=-1) # (A, T)
        states = gt_trajs[:, :, :3].copy() # (A, T, (x, y, yaw))
        
        yaw_rate = wrap_angle(np.diff(yaw.copy(), axis=-1)) / dt
        accel = np.diff(speed.copy(), axis=-1) / dt
        d_states = np.diff(states.copy(), axis=-2)
        d_states[:, :, -1] = wrap_angle(d_states[:, :, -1])
        d_states = d_states / dt
        states[:, :, 2] = wrap_angle(states[:, :, 2])
        
        action_valid = data_sample["obj_trajs_future_mask"][:, :].copy() # (A, T)
        action_valid[:, 1:] = action_valid[:, :-1] * action_valid[:, 1:]

        # filter out invalid actions
        yaw_rate[action_valid==0] = 0.
        accel[action_valid==0] = 0.
        d_states[action_valid==0] = 0.

        # reshape for mean pooling
        num_agents = obj_trajs_future_state.shape[0]
        yaw_rate = yaw_rate.reshape((num_agents, num_actions, -1))
        accel = accel.reshape((num_agents, num_actions, -1))
        d_states = d_states.reshape((num_agents, num_actions, -1, 3))
        action_valid = action_valid.reshape(num_agents, num_actions, -1)

        yaw_rate_sample = yaw_rate.sum(axis=-1) / np.clip(action_valid.sum(axis=-1), 1., np.inf)
        accel_sample = accel.sum(axis=-1) / np.clip(action_valid.sum(axis=-1), 1., np.inf)
        action = np.stack([accel_sample, yaw_rate_sample], axis=-1)
        d_action = d_states.sum(axis=-2) / np.clip(action_valid.sum(axis=-1), 1., np.inf)[:, :, None]
        states = states[:, 1::action_len, :]
        action_valid = action_valid.any(axis=-1)
        
        # Filter again
        action[action_valid==0] = 0.
        d_action[action_valid==0] = 0.
        states[action_valid==0] = 0.
        state_actions = states
        ik_actions = action
        v_actions = d_action
        action_mask = action_valid
        
        data_sample["gt_ik_actions_multi_agent"] = ik_actions
        data_sample["gt_v_actions_multi_agent"] = v_actions
        data_sample["gt_state_actions_multi_agent"] = state_actions
        data_sample["gt_action_mask_multi_agent"] = action_mask
    return



def get_gt_action(output):
    for data_sample in output:
        pass
